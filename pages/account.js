...
function addItemsToCart(items) {
  ...
  items.forEach((it) => {
    ...
    const limit = typeof it.stock === "number" ? Math.max(0, it.stock) : want; // если нет stock — без ограничения
...
export default function AccountPage() {
  ...
  // имитация загрузки заказов: здесь позже подцепим /api/account/orders
  useEffect(() => {
